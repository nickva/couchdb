...
#define USAGE "couchjs [-V|-S stacksize|-h] <script.js>\n"
...
static void add_cx_methods(JSContext* cx) {
  ...
  //JS_AddIntrinsicStringNormalize(cx);
  //JS_AddIntrinsicProxy(cx);
  //JS_AddIntrinsicRegExpCompiler(cx);
  //JS_AddIntrinsicTypedArrays(cx);
  //JS_AddIntrinsicPromise(cx);
  //JS_AddIntrinsicOperators(cx);
}
...
static JSContext* reset_cx(const couch_args* args, JSContext *cx) {
  ...
  obj = JS_ReadObject(cx, args->bytecode, args->bytecode_len,  JS_READ_OBJ_BYTECODE);
...
// Allocates memory. Call free_bytecode() to free it.
//
static void load_bytecode(couch_args* args) {
  size_t len;
  uint8_t* buf = js_load_file(NULL, &len, args->script_path);
  if (buf == NULL) {
    BAIL("Could not script file");
  }

  JSContext* cx = new_cx(args);
  int flags = JS_EVAL_FLAG_COMPILE_ONLY | JS_EVAL_TYPE_GLOBAL;
  JSValue obj = JS_Eval(cx, (const char*)buf, len, args->script_path, flags);
  if (JS_IsException(obj)) {
    BAILJS(cx, "Main script compile error");
  }
  free(buf);

  size_t blen;
  uint8_t* bbuf = JS_WriteObject(cx, &blen, obj, JS_WRITE_OBJ_BYTECODE);
  if (bbuf == NULL) {
    BAILJS(cx, "Bytecode writing error");
  }
  JS_FreeValue(cx, obj);

  // We plan on cleaning up the compile context, so move the bytecode to new
  // area altogether.
  args->bytecode = malloc(blen);
  memcpy(args->bytecode, bbuf, blen);
  args->bytecode_len = blen;
  js_free(cx, bbuf);
  free_cx(cx);
}

static void free_bytecode(couch_args* args) {
  free(args->bytecode);
  args->bytecode = NULL;
  args->bytecode_len = 0;
}
